      ...
      <mt-loadmore :top-method="loadTop" @translate-change="translateChange" @top-status-change="handleTopChange" ref="loadmore">
        <ul class="page-loadmore-list">
          <li v-for="item in list" class="page-loadmore-listitem">
            <img src="../../assets/imgLib/logo.jpg" alt="" style="width: 50px;height: 50px;">
            {{ item.name }}
          </li>
        </ul>
...
<script type="text/babel">
  import  {arrJson} from '@/apps/jsData/findData'
  export default {
    data() {
      return {
        list: [],
      ...
      loadTop() {
        setTimeout(() => {
         /* let firstValue = this.list[0];
          ...
          this.list=arrJson(15);
          console.log('loadTop',this.list)
    ...
    mounted() {
        console.log('height',document.documentElement.clientHeight,this.$refs.wrapper.getBoundingClientRect().top,arrJson());
